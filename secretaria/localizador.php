...
if (isset($_GET['generate_pdf']) && isset($_GET['folio_venta'])) {
    ...
    $folioVentaSeleccionado = $conn->real_escape_string($_GET['folio_venta']);
    $fechaActual = date('Y-m-d H:i:s'); // Obtener fecha y hora actual en formato 'año-mes-día hora:minuto:segundo'
    $clienteSeleccionado = $folioVentaSeleccionado; // Inicializar $clienteSeleccionado con el valor de $folioVentaSeleccionado

    $queryDeudores = "SELECT folio_venta, cliente, direccion, fecha_hora, SUM(subtotal) as total, SUM(cajas) as totalCajas FROM deudores WHERE folio_venta = '$folioVentaSeleccionado' AND DATE(fecha_hora) = DATE('$fechaActual') GROUP BY DATE(fecha_hora), HOUR(fecha_hora)";
    ...
    if ($resultDeudores->num_rows > 0) {
        ...
        $pdf->SetTitle('Notas PDF - Folio de Venta: ' . $folioVentaSeleccionado);
        ...
        while ($rowDeudor = $resultDeudores->fetch_assoc()) {
            ...
            $queryDetalle = "SELECT folio_venta, kilos, piezas, categoria, precio, cajas, subtotal FROM deudores WHERE folio_venta = '$folioVentaSeleccionado' AND DATE(fecha_hora) = DATE('$fechaActual')";
...
<head>
    ...
    <title>Distribuidora | Localizador de Notas</title>
</head>
                        ...
                        <div class="col-md-12">
                            <div class="card">
                                <div class="card-header">
                                    <h3 class="card-title">Localizador de Notas</h3>
                                </div>
                                <div class="card-body">
                                <form action="" method="get">
                                <label for="folio_venta">Busca número de folio:</label>
                                <select name="folio_venta" class="form-control" id="folio_venta">
                                    <?php
                                    // Conectar a la base de datos
                                    require('../config/conexion.php');

                                    // Obtener la lista de folios de venta desde la base de datos
                                    $queryFolios = "SELECT DISTINCT folio_venta FROM deudores";
                                    $resultFolios = $conn->query($queryFolios);

                                    // Generar opciones del menú desplegable
                                    while ($rowFolio = $resultFolios->fetch_assoc()) {
                                        echo "<option value='" . $rowFolio['folio_venta'] . "'>" . $rowFolio['folio_venta'] . "</option>";
                                    }
                                ...
                                <br><br>
                                <label>Aun falta modificar que la busqueda no sea por dia</label>
                                <button type="submit" class="btn btn-primary" name="generate_pdf">Descargar Nota localizada</button>
                            </form>
